"""Run transcription + translation for selected MSP-Podcast partitions."""
...
DEFAULT_OUTPUT_RO_DIR = Path("MSP_Podcast/Transcription_ro")
...
def process_partition(
    ...
    output_ro_dir.mkdir(parents=True, exist_ok=True)
    ...
    for file_id in rows["FileName"].tolist():
        ...
        translation_path = output_ro_dir / f"{key}.txt"
        transcript_path.write_text(text_en, encoding="utf-8")
        translation_path.write_text(text_ro, encoding="utf-8")
        saved_files += 1
        print(f"[OK] {partition}: {key}")

    print(
        f"Saved {saved_files} transcripts to {output_en_dir} and "
        f"{saved_files} translations to {output_ro_dir}"
    )


def run_transcribe_translate(
    config_path: Path = DEFAULT_CONFIG_PATH,
    partitions: Sequence[str] = DEFAULT_PARTITIONS,
    max_files: int | None = DEFAULT_MAX_FILES,
    seed: int = DEFAULT_SEED,
    output_en_dir: Path = DEFAULT_OUTPUT_EN_DIR,
    output_ro_dir: Path = DEFAULT_OUTPUT_RO_DIR,
):
    set_seed(seed)
