...
# Suppress bitsandbytes quantization warnings (expected behavior).
...
class TextOnlyTrainerES:
    ...
    def setup_model_with_lora(self, lora_r: int = 16, lora_alpha: int = 32):
        """Configurează modelul cu QLoRA (8-bit + LoRA pe Atenție + Classifier deblocat)."""
        print("Configuring 8-bit Quantization...")
        
        # 1. Comprimăm la 8-bit, dar protejăm classifier-ul ca să poată învăța
        ...
        model = AutoModelForSequenceClassification.from_pretrained(
            ...
            quantization_config=bnb_config, # Aplicăm cuantizarea!
        )

        

        # 2. Pregătire stabilă pentru antrenament pe 8-biți
        model = prepare_model_for_kbit_training(model)

        model.config.use_cache = False  # Rezolvă conflictul cu checkpointing
        ...
        lora_config = LoraConfig(
            ...
            lora_dropout=0.1,
            bias="none",
            # Acum atacăm toată inima modelului (atenție extinsă) pentru a învăța mai bine
            target_modules=["query", "value", "key", "output.dense", "intermediate.dense"], 
            modules_to_save=["classifier"], # Vrem să actualizăm și decizia finală
        )

        model = get_peft_model(model, lora_config)
        model.print_trainable_parameters()
        trainable_names = [name for name, param in model.named_parameters() if param.requires_grad]
        lora_names = [name for name in trainable_names if "lora" in name.lower()]
        print(f"Trainable parameter groups: {len(trainable_names)}")
        print(f"LoRA params found: {len(lora_names)}")
        if not lora_names:
            print("WARNING: no LoRA parameters found. Check target_modules.")
        else:
    ...
    def train_epoch(
        ...
        # Ponderile calculate pe distribuția: {0: 63693, 1: 65026, 2: 40471}
        ...
        for step, batch in enumerate(progress_bar):
            ...
            # Calculăm Loss-ul ponderat manual
            loss = loss_fn(outputs.logits, labels)
            
            # Îl împărțim dacă facem Gradient Accumulation
            ...
            # Afișăm loss-ul real (neîmpărțit) pe consolă
...
def main():
    ...
    if not transcripts_es_dir.exists():
        raise FileNotFoundError(f"Transcripts directory not found: {transcripts_es_dir}")
    ...
    train_dataset_msp = MSP_Podcast_Dataset(
        ...
        transcripts_en_dir=str(data_dir / "Transcription_en"),
